...
{
    public sealed class TenantOneContext : AbstractTenancyContext
    {
            ...
            (IByteBuffer input) =>
            {
                try
                {
                    ...
                    if (tokens?.Length >= 3)
                    {
                        ...
                        var dataJson = string.Format("{{'ts': '{0}', 'tenantId': '{1}', 'deviceId': '{2}', 'data':'{3}' }}", DateTime.UtcNow, "tenantone", deviceId, dataStr);
                        var dataObj = JObject.Parse(dataJson);
                        var dataBuf = ByteBufferUtil.EncodeString(ByteBufferUtil.DefaultAllocator, dataObj.ToString(), Encoding.UTF8);
